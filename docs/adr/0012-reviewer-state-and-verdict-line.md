...
**Decision (claim identity).** Claim IDs are minted by the analysis when a claim is formed — unique within one analysis document (thread-scoped, e.g. `t2.c3`), embedded in the cockpit's claim elements, and used as the keys of the disposition store. Their stability contract is **within one review run**: across open → `Esc` → `/review-resume` → close the page is never re-authored, so an ID cannot drift. Regeneration produces a new analysis with new IDs — and dispositions are run-scoped and reset with it by design, so no cross-run identity is required. Carrying dispositions across a regeneration (matching old claims to new by content) is a recognized future enhancement, deliberately out of scope here.
