...
3. `lint_cockpit.py … --csp-mode strict` is the post-bake tripwire: it confirms the
   baked artifact is escaped and self-contained (no inline JS, no remote/CDN, strict
   CSP). If it fails, the bake must be fixed before the cockpit is shared.
