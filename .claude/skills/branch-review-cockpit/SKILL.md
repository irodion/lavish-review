...
2. **L0 — Orientation (goal↔implementation)** — `<section class="l0">` with an
   ...
   say the `alignment` partition here in one glance: which threads serve it and which
   are drive-bys (`serves_goal` / `drive_by`). A goal-unserved gap is **not** a
   schema kind (ADR-0016): if one surfaced it rides as an **Attention Note** here —
   `<aside class="attention-note">` with the narrator's note text. One screen that
   answers "what is this branch for and does the work match."
3. **L1/L2 — Threads with their Review Steps** — one
   `<section class="thread" id="t1">` per thread, **in analysis order** (that order
   is the Review Route): an `<h2>` with `<span class="thread-id">t1</span>` and the
   title — plus `<span class="chip flag-drive-by">drive-by</span>` when `alignment`
   lists the thread in `drive_by`, and a **derived**
   `<span class="thread-impacts">` holding the thread's impact counts computed from
   *its own steps* (e.g. "3 behavior-change · 1 test · 5 preserving"). Add an
   `attention-<impact>` class to that span for the most attention-worthy step present
   (`unknown-impact`, else `behavior-change`) so the header tints by the change's
   character. **There is no authored thread impact — you derive these counts from the
   steps; nothing in the pipeline reads a thread impact.** Then the summary as
   `<p class="thread-summary">`, its files as `<p class="thread-paths">` (each path
   is the matching `fragments.json` entry's **`path_html`**, pasted verbatim). Then
   one `<details class="step" id="t1.s1" data-impact="IMPACT">` per step, **in the
   thread's `steps` order** (set `data-impact` to the step's `impact` so the styling
   and the Map's derived character key off it):
   - `<summary>`: an **impact chip** `<span class="chip impact-IMPACT">IMPACT</span>`
     (the step's `impact`), the step's `summary` text, and a confidence chip
     `<span class="chip confidence-LEVEL">confidence: LEVEL</span>` (the step's
     `confidence`). Judgment-color discipline (ADR-0016): write the chip **words**
     exactly as above; the stylesheet adds each chip's glyph from its class —
     **never hand-add a glyph**, it would double, and no impact ever carries a
     checkmark (it is never an approval). `behavior-preserving` renders
     verify-then-skim (no green, no check) with confidence shown prominently;
     `unknown-impact` renders attention-worthy — both from the class alone, so you
     just write the words.
   - `<div class="step-body">`: the `detail` prose (if present) as
     `<p class="detail">`, then the `why_now` as `<p class="why-now">` (why this step
     sits here on the route), then — for a step with `review_prompts` —
     `<h4>Review prompts</h4><ul class="review-prompts">` (one `<li>` per prompt: the
     comparisons/confirmations to make, **not** "what could be wrong"; a step with no
     `review_prompts`, e.g. a `mechanical-change`, omits this block), then
     ...
     When a `{path}` ref carries a **`hunk`** (schema 0.4), link the exact hunk
     ...
     Then, for each of the step's `attention_notes` (if any), a muted
     `<aside class="attention-note">` with the note `text` — and, if the note carries
     `evidence`, its refs as a small `<ul class="evidence-list">` like above. A
     test-change step's `relates_to` targets may be rendered as in-page links
     (`<a href="#t1.s1">documents t1.s1</a>`) — the link is the narrator's, don't
     invent one. Finally plant the step's **live-evidence seam**, empty and exact
     (the id is the step's own):
     `<!--brc:evidence:t1.s1--><!--/brc:evidence:t1.s1-->` — a mid-review answer
     that *is* new evidence is injected here (step 8 e); a step authored without
...
5. **Test runner note** — a small `<section>` with the detected runner/command in
   ...
   clear it is a **suggestion you did not run** — the concrete checks live in the
   steps' `review_prompts`, in their threads.
...
injects the per-step disposition controls (the five-state judgment vocabulary),
the per-thread progress lines, and the per-step step-scoped **ask affordance**
(ADR-0015/0016) itself when the cockpit is served (and correctly renders none in a
portable `file://` copy). The derived thread impact counts are the exception: write
those into the thread `<h2>` yourself (they are static, so the baked `file://`
record shows them offline too).

Render **every** thread and step from the Analysis — don't drop one for brevity,
and render steps you disagree with **faithfully** (ADR-0011: note the
...
and turns on the **structural pass**: the cockpit's step ids must match the
