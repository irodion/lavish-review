...
def step_ids(analysis: object) -> list[str]:
    """Every ``threads[].steps[].id`` in an analysis, in document order (duplicates kept).

    The set the Cockpit Linter (:mod:`branch_review.lint`), Reviewer Dispositions
    (:mod:`branch_review.dispositions`), and live-evidence injection
    (:mod:`branch_review.evidence`) all check the authored DOM against — the steps that
    must each have a ``<details class="step">`` element and a live-evidence seam, and no
    others. Since ADR-0016 this is the sole id walk (the retired 0.3 ``claims`` path is
    gone), so it is a tolerant walk: it guards each level and skips anything malformed
    rather than raising, so a caller that lints an already-validated analysis gets its
    ids and one that passes a rough draft still gets whatever ids are present; order and
    duplicates are preserved so the linter can report a repeated id itself. A document
    with a ``claims`` key (an old-schema analysis) yields no ids — the validator refuses
    it first anyway.
    """
    ids: list[str] = []
