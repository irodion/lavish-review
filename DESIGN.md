...
- **Resume + staleness:** `session.json` carries `{status, base, branch, head_sha, started_at}`. `/review-branch` checks first and offers restore. If `head_sha` advanced → warn and **regenerate by default** (resume-anyway available). v1.1: ambient detection via Lavish's `SessionStart` hook.
