...
- **L0 — Goal alignment.** The Goal Evidence (or its degraded "no stated goal found" notice), the intent summary, the `alignment` partition (which threads serve the goal, which are drive-bys), a **derived route reading-weight budget** ("~90 min at reading pace", heuristic stated), and a **narrated-hunk coverage meter** (issue #104) — what fraction of the changed hunks a Review Step actually anchors ("70 of 397 hunks narrated"), linking into the generated Un-narrated changes queue. The count is derived, never authored (`src/branch_review/coverage.py`); the Map relays the same headline.
