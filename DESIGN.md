...
**Next (PRD'd):** Deck Mode — the map+stage served presentation, judgment-color restyle with a light theme (ADR-0014, ADR-0015). *(Three foundations from this PRD have shipped ahead of the presentation layer — see implemented scope above: the structural lint rules, the **Hunk Anchorer** with `review-analysis/0.3` hunk-anchored evidence, and **claim-scoped questions** on the document-mode claim panels — ADR-0015; the Deck Mode Map/Stage inherits that same ask affordance once it lands.)*
