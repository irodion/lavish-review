...
**Deck Mode**:
The served-only presentation of the Review Cockpit ([ADR-0014](./docs/adr/0014-deck-presentation-mode.md)): a persistent **Map** beside a **Stage** that shows L0 stop zero or one Review Step at a time. Built client-side by the vendored script **from the document's own DOM** (relocating already-escaped nodes — never string-built markup); the L0–L3 document stays the single artifact, the linted form, and the baked `file://` record. Document mode is the only mode on `file://`. **Status:** shipped on the Review Step substrate ([ADR-0016](./docs/adr/0016-guided-change-narration-surface.md), #86/#88): a served review opens in Deck Mode at the rendered L0 orientation; forward navigation enters the Review Route; the Map reuses each thread's renderer-derived impact summary; **relates_to** links stage related steps; keyboard dispositions auto-advance to the next *unreviewed* step; and forward navigation past the last step walks the un-narrated queue as the route's **final act** (issue #105 — bare hunks staged one at a time with their file for context, no disposition control since they are not Review Steps). Document mode remains one lossless toggle away, so the guided walk is the landing experience without forking the artifact.
_Avoid_: app, SPA, queue view, triage mode.

**Map**:
Deck Mode's persistent navigator — the *whole* while the Stage shows the *piece*: threads in Review Route order with one disposition-tinted dot per step, each thread's **derived impact character** (counts computed from its steps — "3 behavior-change · 1 test · 5 preserving" — never an authored thread badge), the changed files with their add/delete stats, overall progress (step coverage; Attention Notes are never dotted or counted), and a **narrated-hunk coverage** readout (issue #104 — the fraction of the diff a Review Step anchors, relayed from L0) that links into the generated Un-narrated changes queue. Clicking any dot (or a thread) stages that step. When any hunk is un-narrated, the Map also carries the **un-narrated act** (issue #105) — the route's final act — with its own **session-scoped "tail walked" progress** (which bare hunks were visited), deliberately kept out of the step-review counts so *steps reviewed* and *tail walked* never conflate. *(Roadmap: a **branch-scoped ask affordance** in the Map — the map-side half of the two-channel chat, [ADR-0015](./docs/adr/0015-claim-scoped-questions.md) — is not yet built; branch questions go through the ordinary loop chat for now.)*
