...
_PAYLOAD = 'Context data: {"kind": "disposition", "claim": "t1.s1", "disposition": "verified"}'
...
def test_parse_structured_payload() -> None:
    prompt = _prompt(f"Disposition set: t1.s1 -> verified {_PAYLOAD}")
    assert parse_disposition_prompt(prompt) == ("t1.s1", "verified")


def test_parse_falls_back_to_prompt_line() -> None:
    prompt = _prompt("Disposition set: t2.s1 -> question-open")
    assert parse_disposition_prompt(prompt) == ("t2.s1", "question-open")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'Context data: {"kind": "other", "claim": "t1.s1", "disposition": "verified"}',
        'Context data: {"kind": "disposition", "claim": "t1.s1", "disposition": "<script>"}',
...
def test_apply_updates_unreviewed_removes_entry() -> None:
    valid = claim_ids(_ANALYSIS)
    state, rejected = apply_updates({"t1.s1": "verified"}, [("t1.s1", "unreviewed")], valid)
...
def test_apply_updates_drops_stale_keys_from_current() -> None:
    ...
    valid = claim_ids(_ANALYSIS)
    state, _ = apply_updates({"t9.s9": "verified", "t1.s2": "concern"}, [], valid)
    assert state == {"t1.s2": "concern"}


def test_progress_counts_per_thread() -> None:
    rows = progress(_ANALYSIS, {"t1.s1": "verified", "t1.s2": "concern"})
...
def test_store_roundtrip_and_tolerant_load(tmp_path: Path) -> None:
    ...
    save_dispositions(path, {"t1.s1": "verified"})
    assert load_dispositions(path) == {"t1.s1": "verified"}
...
_TOON_DISPOSITION = (
    ...
    '  "7","Disposition set: t1.s1 -> verified Context data: '
    '{\\"kind\\": \\"disposition\\", \\"claim\\": \\"t1.s1\\", '
...
def test_apply_folds_poll_into_store(tmp_path: Path) -> None:
    ...
    assert applied == [("t1.s1", "verified")] and rejected == []
    assert load_dispositions(out / DISPOSITIONS_NAME) == {"t1.s1": "verified"}
...
def test_apply_decodes_escaped_multiline_prompt(tmp_path: Path) -> None:
    ...
    toon = (
        ...
        '  "9","Disposition set: t2.s1 -> question-open\\n\\nContext data:\\n'
        '{\\n  \\"kind\\": \\"disposition\\",\\n  \\"claim\\": \\"t2.s1\\",\\n'
    ...
    applied, rejected = apply(out)
    assert applied == [("t2.s1", "question-open")] and rejected == []
    assert load_dispositions(out / DISPOSITIONS_NAME) == {"t2.s1": "question-open"}
...
def test_cli_apply_and_show(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "applied: t1.s1 -> verified" in captured.out

    assert main(["show", "--out", str(out)]) == 0
    captured = capsys.readouterr()
    assert "t1.s1: verified" in captured.out
