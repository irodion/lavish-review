"""Tests for live evidence fragment injection (issue #43, step-shaped by #86).

The acceptance criteria are pinned directly: an escaped, linted fragment lands at
the right step's seam; a lint failure blocks the write entirely (nothing on disk
...
<details class="step" id="t1.s1"><summary>the step</summary>
<div class="step-body"><p class="detail">detail</p>
{_SEAM}
...
def _fragment(body: str, *, title: str = "Callers", step: str = "t1.s1") -> EvidenceFragment:
    return EvidenceFragment(
        step=step, seq=1, ts="2026-07-03T00:00:00+00:00", title=title, body=body
    )
...
def test_render_escapes_hostile_body_and_title() -> None:
    out = render_step_evidence([_fragment("<script>alert(1)</script>", title="a <b> title")])
...
def test_render_empty_is_empty() -> None:
    assert render_step_evidence([]) == ""


def test_evidence_seam_rejects_non_step_ids() -> None:
    with pytest.raises(ValueError):
        evidence_seam("--><script>")


def test_injection_touches_only_the_seam_and_is_idempotent() -> None:
    content = render_step_evidence([_fragment("+ new line")])
...
def test_add_evidence_injects_escaped_and_lint_clean(cockpit: Path) -> None:
    ...
    assert len(fragments) == 1 and fragments[0].step == "t1.s1"


def test_injection_runs_structural_lint_with_step_ids(cockpit: Path) -> None:
    # Passing the analysis step set turns on the structural pass (issue #62); the
    # fixture's step set (t1.s1) matches, so injection still succeeds.
    assert add_evidence(cockpit, "t1.s1", "Callers", "+ x\n", step_ids=["t1.s1"]) == []


def test_injection_blocked_when_structural_pass_fails(cockpit: Path) -> None:
    # A step set declaring t1.s2 (which the fixture never authored): the structural
    # pass fails (missing element + seam), so nothing is written — the chat-only floor.
    errors = add_evidence(cockpit, "t1.s1", "Callers", "+ x\n", step_ids=["t1.s1", "t1.s2"])
    assert errors and any("step-id-missing" in e or "seam-missing" in e for e in errors)
...
def test_cli_loads_sibling_analysis_for_the_structural_pass(
    ...
    assert "step-id-missing" in capsys.readouterr().err
    assert cockpit.read_text(encoding="utf-8") == _COCKPIT  # untouched
...
def test_cli_degrades_when_default_sibling_analysis_is_corrupt(cockpit: Path) -> None:
    ...
    # _load_step_ids returns None (its JSONDecodeError branch), the structural pass is
...
@pytest.mark.parametrize(
    ("step", "title", "body", "needle"),
    [
        ("../../etc", "t", "b", "not a step id"),
...
def test_add_evidence_refuses_bad_input_and_writes_nothing(
    cockpit: Path, step: str, title: str, body: str, needle: str
) -> None:
    errors = add_evidence(cockpit, step, title, body)
...
def test_non_numeric_seq_falls_back_instead_of_crashing(tmp_path: Path) -> None:
    ...
    payload = {
        "schema": "review-live-evidence/0.2",
        "fragments": [{"step": "t1.s1", "seq": "x", "ts": "t", "title": "T", "body": "b"}],
    }
...
def test_cli_records_are_json_readable(cockpit: Path) -> None:
    ...
    assert payload["schema"] == "review-live-evidence/0.2"
    assert payload["fragments"][0]["step"] == "t1.s1"
