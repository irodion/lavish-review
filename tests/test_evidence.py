...
def test_add_evidence_injects_escaped_and_lint_clean(tmp_path: Path) -> None:
...
def test_add_evidence_accumulates_without_duplicating(tmp_path: Path) -> None:
    cockpit = tmp_path / "review.html"
    cockpit.write_text(_COCKPIT, encoding="utf-8")
    ...
    assert [f.seq for f in load_fragments(tmp_path / EVIDENCE_NAME)] == [1, 2]
...
def test_add_evidence_refuses_bad_input_and_writes_nothing(
    tmp_path: Path, claim: str, title: str, body: str, needle: str
) -> None:
    cockpit = tmp_path / "review.html"
    cockpit.write_text(_COCKPIT, encoding="utf-8")
...
def test_injected_evidence_survives_the_bake(tmp_path: Path) -> None:
...
def test_cli_success_and_floor(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    cockpit = tmp_path / "review.html"
    cockpit.write_text(_COCKPIT, encoding="utf-8")
...
def test_cli_records_are_json_readable(tmp_path: Path) -> None:
    cockpit = tmp_path / "review.html"
    cockpit.write_text(_COCKPIT, encoding="utf-8")
    assert add_evidence(cockpit, "t1.c1", "T", "+ b\n") == []
    payload = json.loads((tmp_path / EVIDENCE_NAME).read_text(encoding="utf-8"))
